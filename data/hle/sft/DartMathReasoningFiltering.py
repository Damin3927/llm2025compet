...
def parse_args():
    ...
    parser.add_argument('--inference-model', type=str, default="Qwen/Qwen-32B",
    ...
    parser.add_argument('--inference-max-model-len', type=int, default=20000,
    ...
    parser.add_argument('--judgement-model', type=str, default="Qwen/Qwen-32B",
    ...
    parser.add_argument('--judgement-max-model-len', type=int, default=20000,
...
def vllm_inference(llm, prompts, temperature=0.3, max_tokens=20000):
...
def vllm_judgement(llm, prompts, temperature=0.1, max_tokens=20000):
    sampling_params = SamplingParams(
        temperature=temperature,
        max_tokens=max_tokens,
        n=1,  # number of completions per prompt
...
def inference(llm, inf_dataset, inference_batch_size, save_per_batch, inference_temperature, inference_max_tokens, inference_prompt, inference_dir, dataset_size):
    ...
    for data_batch in tqdm(inf_dataset.iter(batch_size=inference_batch_size), desc="Inferencing"):
        ...
        inference_prompts = [inference_prompt.format(question=question) for question in data_batch["query"]]
        ...
        actual_batch_size = len(data_batch['query'])  # Get actual size of current batch
...
def judgement(jud_model, judgement_batch_size, judgement_temperature, judgement_max_tokens, judgement_prompt, inference_dir, judgement_dir):
    ...
    for inf_filename in tqdm(os.listdir(inference_dir), desc="Judging"):
        ...
        for index in range(0, num_rows, judgement_batch_size):
            ...
            question = [item['query'] for item in batch]
            correct_answer = [item["response"] for item in batch]
...
cot_dataset = datasets.load_dataset("hkust-nlp/dart-math-hard", streaming=True)
