...
プロジェクトルートのREADME.mdに従う。
（オリジナルコードを動かすにはsklearnが必要だが、ここではコードの改修でカバー。）

# 環境変数
do_not_answer/utils/info.yaml に環境変数をセット。
```dotenv
OpenAI: 'xxxx'
HuggingFace: 'xxxx'
Anthropic: YOURKEY
...
# 実行例
```bash
python evaluation/eval_dna/llm-compe-eval/evaluate_huggingface_models.py --model_name Qwen/Qwen3-0.6B --max_questions 10
```
...
**gpt.py**
- openai.api_key をセットする行（2行）をコメントアウト。
```python
